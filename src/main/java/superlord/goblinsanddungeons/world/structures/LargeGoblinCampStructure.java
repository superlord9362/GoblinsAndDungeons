...
public class LargeGoblinCampStructure extends Structure<NoFeatureConfig> {
	...
	public LargeGoblinCampStructure(Codec<NoFeatureConfig> config) {
		super(config);
	}
	...
	public static class Piece extends TemplateStructurePiece {
		...
		private boolean gob4;
		private boolean garch;
		private boolean garch2;
		private boolean garch3;
		private boolean goblo;
		private boolean hobgob;
		...
		public Piece(TemplateManager templateManagerIn, CompoundNBT tagCompound) {
			...
			this.gob4 = tagCompound.getBoolean("Gob4");
			this.garch = tagCompound.getBoolean("Garch");
			this.garch2 = tagCompound.getBoolean("Garch2");
			this.garch3 = tagCompound.getBoolean("Garch3");
			this.goblo = tagCompound.getBoolean("Goblo");
			this.hobgob = tagCompound.getBoolean("Hobgob");
		}
		...
		public static void start(TemplateManager templateManager, BlockPos pos, Rotation rotation2, List<StructurePiece> pieceList, Random rand) {
			...
			pieceList.add(new Piece(templateManager, new ResourceLocation(GoblinsAndDungeons.MOD_ID, "large_goblin_camp"), blockPos, rotation2));
		}
		...
		protected void readAdditional(CompoundNBT tagCompound) {
			...
			tagCompound.putBoolean("Gob4", this.gob4);
			tagCompound.putBoolean("Garch", this.garch);
			tagCompound.putBoolean("Garch2", this.garch2);
			tagCompound.putBoolean("Garch3", this.garch3);
			tagCompound.putBoolean("Goblo", this.goblo);
			tagCompound.putBoolean("Hobgob", this.hobgob);
		}
		...
		public boolean func_230383_a_(ISeedReader seedReader, StructureManager structureManager, ChunkGenerator chunkGenerator, Random random, MutableBoundingBox structureBoundingBoxIn, ChunkPos chunkPos, BlockPos pos) {
			...
			if (!this.hobgob) {
				...
				if (structureBoundingBoxIn.isVecInside(new BlockPos(l, pos.getY(), k))) {
					this.goblo = true;
					HobGobEntity hobgobentity = EntityInit.HOBGOB.get().create(seedReader.getWorld());
					hobgobentity.enablePersistence();
					hobgobentity.setLocationAndAngles((double)l + 0.5D, (double)pos.getY(), (double)k + 0.5D, 0.0F, 0.0F);
					hobgobentity.onInitialSpawn(seedReader, seedReader.getDifficultyForLocation(new BlockPos(l, pos.getY(), k)), SpawnReason.STRUCTURE, (ILivingEntityData)null, (CompoundNBT)null);
					seedReader.func_242417_l(hobgobentity);
				}
			}
