...
public class LockedChestTileEntity extends LockableLootTileEntity implements IChestLid, ITickableTileEntity {
	...
	protected LockedChestTileEntity(TileEntityType<?> typeIn) {
		super(typeIn);
	}

	public LockedChestTileEntity() {
		this(TileEntityInit.LOCKED_CHEST.get());
	}
	...
	private void playSound(SoundEvent soundIn) {
		ChestType chesttype = this.getBlockState().get(LockedChestBlock.TYPE);
		if (chesttype != ChestType.LEFT) {
			double d0 = (double)this.pos.getX() + 0.5D;
			double d1 = (double)this.pos.getY() + 0.5D;
			double d2 = (double)this.pos.getZ() + 0.5D;
			if (chesttype == ChestType.RIGHT) {
				Direction direction = LockedChestBlock.getDirectionToAttached(this.getBlockState());
	...
	public static int getPlayersUsing(IBlockReader reader, BlockPos posIn) {
		...
		if (blockstate.hasTileEntity()) {
			...
			if (tileentity instanceof LockedChestTileEntity) {
				return ((LockedChestTileEntity)tileentity).numPlayersUsing;
			}
		}
	...
	public static void swapContents(LockedChestTileEntity chest, LockedChestTileEntity otherChest) {
	...
	private net.minecraftforge.items.IItemHandlerModifiable createHandler() {
		...
		if (!(state.getBlock() instanceof LockedChestBlock)) {
			return new net.minecraftforge.items.wrapper.InvWrapper(this);
		}
		IInventory inv = LockedChestBlock.getChestInventory((LockedChestBlock) state.getBlock(), state, getWorld(), getPos(), true);
