 ...
 */
@OnlyIn(Dist.CLIENT)
public class GobberModel<T extends Entity> extends EntityModel<GobberEntity> implements IHasArm {
	...
	public void setRotationAngles(GobberEntity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		if (entityIn.isSleeping()) {
			this.RightPauldron.rotateAngleZ = -0.5F * 0.45F * MathHelper.sin((0.6F * ageInTicks) / 10);
			this.LeftPauldron.rotateAngleZ = -(-0.5F * 0.45F * MathHelper.sin((0.6F * ageInTicks) / 10));
			...
			this.RightPauldron.rotateAngleZ = 0.0F;
			this.LeftPauldron.rotateAngleZ = 0.0F;
	...
	public void translateHand(HandSide sideIn, MatrixStack matrixStackIn) {
		...
		modelrenderer.rotationPointX -= f;
		matrixStackIn.translate(-0.0, 0.45, 0);
	}
