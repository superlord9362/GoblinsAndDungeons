...
public class GobloModel<T extends Entity> extends EntityModel<GobloEntity> implements IHasArm {
	...
	public void setRotationAngles(GobloEntity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		this.Head.rotateAngleX = -MathHelper.abs(MathHelper.cos(limbSwing * 0.3331F) * 0.35F * limbSwingAmount);
		ItemStack itemstack = entityIn.getItemStackFromSlot(EquipmentSlotType.MAINHAND);

		if (!itemstack.isEmpty()) {
			for(int i = 0; i < 8; ++i) {
				this.Head.rotateAngleX = -MathHelper.abs(-0.5F * 1F * MathHelper.sin((1.5F * ageInTicks) / 10));
			}
		}
	}
