...
public class SoulAshCampfireBlock extends ContainerBlock implements IWaterLoggable {
	...
	public SoulAshCampfireBlock(boolean smokey, int fireDamage, AbstractBlock.Properties properties) {
		super(properties);
		this.smokey = smokey;
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
		...
		if (tileentity instanceof SoulAshCampfireTileEntity) {
			SoulAshCampfireTileEntity SoulAshCampfireTileEntity = (SoulAshCampfireTileEntity)tileentity;
			ItemStack itemstack = player.getHeldItem(handIn);
			Optional<CampfireCookingRecipe> optional = SoulAshCampfireTileEntity.findMatchingRecipe(itemstack);
			if (optional.isPresent()) {
				if (!worldIn.isRemote && SoulAshCampfireTileEntity.addItem(player.abilities.isCreativeMode ? itemstack.copy() : itemstack, optional.get().getCookTime())) {
	...
	public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
		if (!state.isIn(newState.getBlock())) {
			TileEntity tileentity = worldIn.getTileEntity(pos);
			if (tileentity instanceof SoulAshCampfireTileEntity) {
				InventoryHelper.dropItems(worldIn, pos, ((SoulAshCampfireTileEntity)tileentity).getInventory());
			}
	...
	public static void extinguish(IWorld world, BlockPos pos, BlockState state) {
		...
		if (tileentity instanceof SoulAshCampfireTileEntity) {
			((SoulAshCampfireTileEntity)tileentity).dropAllItems();
		}
	...
	public TileEntity createNewTileEntity(IBlockReader worldIn) {
		return new SoulAshCampfireTileEntity();
	}
